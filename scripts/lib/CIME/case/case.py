...
class Case(object):
    ...
    def _valid_compset_impl(self, compset_name, comp_classes, comp_hash):
        ...
        ('2000_DATM%NYF_SLND_DICE%SSMI_DOCN%DOM_DROF%NYF_SGLC_SWAV_SESP', ['2000', 'DATM%NYF', 'SLND', 'DICE%SSMI', 'DOCN%DOM', 'DROF%NYF', 'SGLC', 'SWAV', 'SESP'])
        >>> Case(read_only=False)._valid_compset_impl('2000_DATM%NYF_SLND_DICE%SSMI_DOCN%SOM_DROF%NYF_SGLC_SWAV_TEST', ['CPL', 'ATM', 'LND', 'ICE', 'OCN', 'ROF', 'GLC', 'WAV', 'ESP'], {'datm':1,'satm':1,'dlnd':2,'slnd':2,'dice':3,'sice':3,'docn':4,'socn':4,'drof':5,'srof':5,'sglc':6,'swav':7,'ww3':7,'sesp':8})
        ('2000_DATM%NYF_SLND_DICE%SSMI_DOCN%SOM_DROF%NYF_SGLC_SWAV_SESP_TEST', ['2000', 'DATM%NYF', 'SLND', 'DICE%SSMI', 'DOCN%SOM', 'DROF%NYF', 'SGLC', 'SWAV', 'SESP'])
        ...
        bgc = None # BGC is a special case
        test = None # TEST is another special case
        ...
        # Add optional BGC, if present
        if bgc is not None:
            compsetname = compsetname + '_' + bgc
        # Add optional TEST, if present
        if test is not None:
            compsetname = compsetname + '_' + test
